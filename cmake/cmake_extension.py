...
def is_for_pypi():
    ans = os.environ.get("KALDI_DECODER_IS_FOR_PYPI", None)
...
try:
    ...
    class bdist_wheel(_bdist_wheel):
        def finalize_options(self):
            ...
            # kaldi-decoder-xxx-pyxx-none-any.whl
...
class BuildExtension(build_ext):
    def build_extension(self, ext: setuptools.extension.Extension):
        ...
        install_dir = Path(self.build_lib).resolve() / "kaldi_decoder"

        kaldi_decoder_dir = Path(__file__).parent.parent.resolve()

        cmake_args = os.environ.get("KALDI_DECODER_CMAKE_ARGS", "")
        make_args = os.environ.get("KALDI_DECODER_MAKE_ARGS", "")
        ...
        if is_windows():
            build_cmd = f"""
         cmake {cmake_args} -B {self.build_temp} -S {kaldi_decoder_dir}
         cmake --build {self.build_temp} --target install --config Release -- -m
            """
            print(f"build command is:\n{build_cmd}")
            ret = os.system(
                f"cmake {cmake_args} -B {self.build_temp} -S {kaldi_decoder_dir}"
            )
            if ret != 0:
                raise Exception("Failed to configure kaldi-decoder")

            ret = os.system(
                f"cmake --build {self.build_temp} --target install --config Release -- -m"  # noqa
            )
            if ret != 0:
                raise Exception("Failed to build and install kaldi-decoder")
        else:
            ...
            build_cmd = f"""
                ...
                cmake {cmake_args} {kaldi_decoder_dir}
