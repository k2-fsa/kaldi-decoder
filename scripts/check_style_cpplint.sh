...
kaldi_decoder_dir=$(cd $cur_dir/.. && pwd)

build_dir=$kaldi_decoder_dir/build
mkdir -p $build_dir
...
source $kaldi_decoder_dir/scripts/utils.sh
...
function do_check() {
  case "$1" in
    ...
    2)
      ...
      files=$(find $kaldi_decoder_dir/kaldi-decoder -name "*.h" -o -name "*.cc" -o -name "*.cu")
      ;;
    *)
